...
RSpec.describe "BrsScales", type: :request do
  ...
  describe "#new" do
    context "login as admin" do
      ...
      it "show own patient new brs scale page" do
        get new_patient_brs_scales_path admin_patient
        expect(response).to render_template :new
      end

      it "show other patient new brs scale page" do
        get new_patient_brs_scales_path therapist_patient
        expect(response).to render_template :new
      end
    end
  ...
  describe "#edit" do
    ...
    context "login as admin" do
      ...
      it "show edit page in own patient page" do
        get edit_patient_brs_scales_path admin_patient
        expect(response).to render_template :edit
      end

      it "show edit page in other patient page" do
        get edit_patient_brs_scales_path therapist_patient
        expect(response).to render_template :edit
      end
    end
  ...
  describe "#update" do
    ...
    let!(:brs_scale_params) do
      attributes_for(:brs_scale, upper_limbs: "stage_one",
                                 finger: "stage_three",
    ...
    context "login as admin" do
      ...
      it "is success to update own patient brs" do
        patch patient_brs_scales_path(admin_patient), params: {
          brs_scale: brs_scale_params,
        }
        admin_patient_brs.reload
        expect(admin_patient_brs.upper_limbs).to eq "stage_one"
        expect(admin_patient_brs.finger).to eq "stage_three"
        expect(admin_patient_brs.lower_limbs).to eq "stage_six"
      end
      ...
      it "is success to update other patient brs" do
        patch patient_brs_scales_path(therapist_patient), params: {
          brs_scale: brs_scale_params,
        }
        therapist_patient_brs.reload
        expect(therapist_patient_brs.upper_limbs).to eq "stage_one"
        expect(therapist_patient_brs.finger).to eq "stage_three"
        expect(therapist_patient_brs.lower_limbs).to eq "stage_six"
      end
    end
    ...
    context "login as therapist" do
      ...
      it "is false to update other patient brs" do
        ...
        admin_patient_brs.reload
        expect(admin_patient_brs.upper_limbs).not_to eq "stage_one"
        expect(admin_patient_brs.finger).not_to eq "stage_three"
        expect(admin_patient_brs.lower_limbs).not_to eq "stage_six"
      end
  ...
  describe "#destroy" do
    ...
    context "login as admin" do
      ...
      it "is success to destor own patient brs" do
        expect do
          delete patient_brs_scales_path admin_patient
        end.to change(BrsScale, :count).by(-1)
      end

      it "is success to destor other patient brs" do
        expect do
          delete patient_brs_scales_path therapist_patient
        end.to change(BrsScale, :count).by(-1)
      end
