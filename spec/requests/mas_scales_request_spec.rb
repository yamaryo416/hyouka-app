...
RSpec.describe "MasScales", type: :request do
  ...
  describe "#new" do
    context "login as admin" do
      ...
      it "show own patient new mas scale page" do
        get new_patient_mas_scales_path admin_patient
        expect(response).to render_template :new
      end

      it "show other patient new mas scale page" do
        get new_patient_mas_scales_path therapist_patient
        expect(response).to render_template :new
      end
    end
  ...
  describe "#edit" do
    ...
    context "login as admin" do
      ...
      it "show edit page in own patient page" do
        get edit_patient_mas_scales_path admin_patient
        expect(response).to render_template :edit
      end

      it "show edit page in other patient page" do
        get edit_patient_mas_scales_path therapist_patient
        expect(response).to render_template :edit
      end
    end
  ...
  describe "#update" do
    ...
    let!(:mas_scale_params) do
      attributes_for(:mas_scale, elbow_joint: "mild_enhancement",
                                 wrist_joint: "rigidity",
    ...
    context "login as admin" do
      ...
      it "is success to update own patient mas" do
        patch patient_mas_scales_path(admin_patient), params: {
          mas_scale: mas_scale_params,
        }
        admin_patient_mas.reload
        expect(admin_patient_mas.elbow_joint).to eq "mild_enhancement"
        expect(admin_patient_mas.wrist_joint).to eq "rigidity"
        expect(admin_patient_mas.knee_joint).to eq "significant"
      end
      ...
      it "is success to update other patient mas" do
        patch patient_mas_scales_path(therapist_patient), params: {
          mas_scale: mas_scale_params,
        }
        therapist_patient_mas.reload
        expect(therapist_patient_mas.elbow_joint).to eq "mild_enhancement"
        expect(therapist_patient_mas.wrist_joint).to eq "rigidity"
        expect(therapist_patient_mas.knee_joint).to eq "significant"
      end
    end
    ...
    context "login as therapist" do
      ...
      it "is false to update other patient mas" do
        ...
        admin_patient_mas.reload
        expect(admin_patient_mas.elbow_joint).not_to eq "mild_enhancement"
        expect(admin_patient_mas.wrist_joint).not_to eq "rigidity"
        expect(admin_patient_mas.knee_joint).not_to eq "significant"
      end
    ...
    context "not login" do
      it "false to update patient mas" do
        ...
        admin_patient_mas.reload
        expect(therapist_patient_mas.elbow_joint).not_to eq "mild_enhancement"
        expect(therapist_patient_mas.wrist_joint).not_to eq "rigidity"
        expect(therapist_patient_mas.knee_joint).not_to eq "significant"
      end
  ...
  describe "#destroy" do
    ...
    context "login as admin" do
      ...
      it "is success to destor own patient mas" do
        expect do
          delete patient_mas_scales_path admin_patient
        end.to change(MasScale, :count).by(-1)
      end

      it "is success to destor other patient mas" do
        expect do
          delete patient_mas_scales_path therapist_patient
        end.to change(MasScale, :count).by(-1)
      end
